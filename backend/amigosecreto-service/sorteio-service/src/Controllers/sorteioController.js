...
module.exports = {
    async sortear (request, response){
        try {
            ...
            let i=0;
            for(let i=0;i<participantes.lenght; i++){
                let auxIndex = novoArray.findIndex(item => item.idParticipante !== participantes[1]._idParticipante);
                participantes[i]._idAmigo = novoArray[auxIndex].idParticipante;
                participantes[i].nomeAmigo = novoArray[auxIndex].nomeParticipante;
                novoArray = novoArray.filter( item => item.idParticipante !== participantes._idAmigo);
            }
